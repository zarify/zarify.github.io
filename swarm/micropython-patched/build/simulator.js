...
(() => {
  ...
  var Audio = class {
    ...
    async createAudioContextFromUserInteraction() {
      var _a;
      this.context = (_a = this.context) != null ? _a : new (window.AudioContext || window.webkitAudioContext)({
        sampleRate: 44100
      });
    ...
    playSoundExpression(expr) {
      ...
      const callback = () => {
        ...
        if (this.context) {
          const target = this.context.createBuffer(1, source.length, synth.sampleRate);
          const channel = target.getChannelData(0);
  ...
  var Display = class {
    ...
    render() {
      ...
      (_a = this.onChange) == null ? void 0 : _a.call(this, {
        displayPixels: this.getDisplayPixels()
      });
    }
    boardStopped() {
      this.clear();
    ...
    if (e.source === window.parent) {
      ...
      switch (data.kind) {
        ...
        case "radio_input": {
          ...
          board2.radio.receive(data.data, data.rssi);
          break;
        }
        case "set_value": {
          const { id, value } = data;
  ...
  // flags.ts
  var allFlags = [{ name: "sw", defaultOnStages: [] }];
  var flagsForParams = (stage2, params) => {
    const enableFlags = new Set(params.getAll("flag"));
    const allFlagsDefault = enableFlags.has("none") ? false : enableFlags.has("*") ? true : void 0;
    return Object.fromEntries(allFlags.map((f) => [
      f.name,
      isEnabled(f, stage2, allFlagsDefault, enableFlags.has(f.name))
    ]));
  };
  var isEnabled = (f, stage2, allFlagsDefault, thisFlagOn) => {
    if (thisFlagOn) {
      return true;
    }
    if (allFlagsDefault !== void 0) {
      return allFlagsDefault;
    }
    return f.defaultOnStages.includes(stage2);
  };
  var flags = (() => {
    const params = new URLSearchParams(window.location.search);
    return flagsForParams(stage, params);
  })();
  ...
  if ("serviceWorker" in navigator && flags.sw) {
    initServiceWorker();
  }
