...
class Parser {
        ...
        while (this.match(TokenType.EXCEPT)) {
            ...
            if (this.match(TokenType.STAR)) {
                ...
                if (!this.check(TokenType.COLON)) {
                    type = this.parseTest();
                    if (this.match(TokenType.AS)) {
                        name = this.consume(TokenType.NAME, "Expected name after 'as'").value;
                    }
                }
            }
            else {
            ...
            if (!this.check(TokenType.RBRACE)) {
                do {
                    if (this.match(TokenType.DOUBLESTAR)) {
                        // **rest pattern
                        rest = this.consume(TokenType.NAME, "Expected name after '**'").value;
                        break;
                    }
    ...
    parseShiftExpr() {
        ...
        while (this.match(TokenType.LEFTSHIFT, TokenType.RIGHTSHIFT)) {
            ...
            const op = opToken.type === TokenType.LEFTSHIFT
                ? { nodeType: "LShift" }
                : { nodeType: "RShift" };
    ...
    parseTerm() {
        ...
        while (this.match(TokenType.STAR, TokenType.AT, TokenType.SLASH, TokenType.DOUBLESLASH, TokenType.PERCENT)) {
            const opToken = this.previous();
            let op;
            switch (opToken.type) {
                case TokenType.STAR:
    ...
    parseAtomWithTrailers() {
        ...
        while (true) {
            if (this.match(TokenType.DOT)) {
                const attr = this.consume(TokenType.NAME, "Expected attribute name").value;
                expr = {
                    nodeType: "Attribute",
                    value: expr,
                    attr,
    ...
    parseAtom() {
        ...
        if (this.match(TokenType.LPAR)) {
            ...
            const isAsyncGenerator = this.check(TokenType.ASYNC) && this.checkNext(TokenType.FOR);
        ...
        if (!this.check(TokenType.RPAR)) {
            do {
                ...
                if (this.match(TokenType.SLASH)) {
                    ...
                    const name = this.consume(TokenType.NAME, "Expected parameter name").value;
                    let annotation;
                    if (this.match(TokenType.COLON)) {
                        annotation = this.parseTestOrStarred();
                    }
    ...
    // ==== Utility methods ====
    matchAugAssign() {
        return (this.check(TokenType.PLUSEQUAL) ||
    ...
    matchComparison() {
        return (this.check(TokenType.LESS) ||
    ...
    parseCompOp() {
        ...
        if (this.match(TokenType.GREATER))
            return { nodeType: "Gt" };
        if (this.match(TokenType.EQEQUAL))
            return { nodeType: "Eq" };
    ...
    parseTypeParams() {
        ...
        do {
            ...
            if (this.match(TokenType.DOUBLESTAR)) {
                const name = this.consume(TokenType.NAME, "Expected parameter name after '**'").value;
                let default_value;
                if (this.match(TokenType.EQUAL)) {
            ...
            // Regular TypeVar (T, T: bound, T = default)
                ...
                const name = this.consume(TokenType.NAME, "Expected type parameter name").value;
                let bound;
                let default_value;
